...
| TC-05 | Empty required field(s)  | Leave one or more fields blank. Submit.                                                      | Form should not submit / validation error shown.                             |Account creation successful with missing input in each field| ❌ Fail |
...
|BUG-02|Signup successful with missing input in required fields|Submit signup form with missing email, then again with missing username and password|Expected: validation error returned. Actual: account created successfully with missing fields|High; allows incomplete accounts into the database|Open|
